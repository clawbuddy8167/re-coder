REBOL [
    Title:   {Re Coder Agent (Rebol3) — Streaming Edition}
    Name:    're-coder-agent-stream
    Author:  {Hermes Agent}
    Version: 3.0.0
    Rights:  {MIT}
    Purpose: {A minimal code agent with LLM client, tool registry, agent loop,
              and SSE streaming support. Written for Rebol3 (Rebol/Bulk 3.21.0+).
...
config: make object! [
    ...
    ; SSE streaming mode
    stream-mode: true             ; Enable streaming by default
    stream-show-tokens: true      ; Show tokens as they arrive
]
...
; ═══════════════════════════════════════════════════════════
;  HTTP POST helper (Non-streaming, legacy)
;
...
; ═══════════════════════════════════════════════════════════
;  LLM Client (Supports both streaming and non-streaming)
; ═══════════════════════════════════════════════════════════
...
llm-client: make object! [
    ...
    chat-stream: func [
        messages [block!]
        callback [function!]  ; func [token [string!]]
        /with-tools tool-defs [block!]
        /local url payload headers
    ][
        url: to-url rejoin [config/base-url {/chat/completions}]
...
code-agent: make object! [
    ...
    run: func [user-prompt [string!]] [
        ...
        repeat turn config/max-turns [
            ...
            response: either config/stream-mode [
                run-streaming-turn messages
            ][
                run-nonstreaming-turn messages
            ]
            ...
            msg: either map? response [
                ; For streaming, response is already the message map
                response
            ][
                ; For non-streaming, extract from choices
    ...
    ; Streaming turn implementation
    run-streaming-turn: func [messages [block!] /local response content tool-calls usage][
        ; Initialize result
        content: copy ""
        tool-calls: copy []
        usage: none
        ...
        ; Build message map from streamed response
        msg: make map! reduce [
            to-set-word 'role {assistant}
            to-set-word 'content select response 'content
    ...
    ; Non-streaming turn (legacy)
    run-nonstreaming-turn: func [messages [block!] /local response][
        llm-client/chat/with-tools messages registry/specs
    ]
]
...
main: func [] [
    unless config/api-key [
        ...
        print {Usage: DEEPSEEK_API_KEY=*** rebol3 re-coder-agent-stream.r3 <prompt>}
        print {   or: DEEPSEEK_API_KEY=*** rebol3 re-coder-agent-stream.r3 --model deepseek-chat --base-url <url> {Write a Node.js quicksort}}
        quit/return 1
    ]
    ...
    while [i <= length? args] [
        ...
        switch/default arg [
            ...
            {--no-show-tokens} [
                config/stream-show-tokens: false
            ]
        ][
    ...
    ; Banner
    print [newline {■ Re Coder Agent (Rebol3) — Streaming Edition}]
    print [{  Model:    } config/model]
    print [{  API:      } config/base-url]
    print [{  Dir:      } config/work-dir]
    print [{  Stream:   } either config/stream-mode [{ON}][{OFF}]]
